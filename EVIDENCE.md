...
| 116 | A constant datum offset explains it: `Delta_hr*K^y - C` gives C = 11.3 K, y = 0.98 and residual **0.99 K**, against **4.64 K** for the IEC form with no offset, whose exponent rails | **(b)** | `offset_hypothesis.py`. **Reading corrected 3 Sept 2026 — see claims 126-127.** The original wording "do not share a reference point" was put to the dataset author, who read it as a claimed calibration error and rejected that. The mechanism is sensor HEIGHT in the oil column, not calibration |
...
| 125 | The dataset author states the **HV rated current is 175 A**. The 132 kV base used here derives 174.95 A, a **0.03 %** difference, so the **0.048-0.429 pu** load hull is confirmed rather than assumed | **(a)** | Correspondence, 3 Sept 2026. Upgrades the central negative finding from (b) to (a): the replication gap does not close, on the owner's own rating |
| 126 | The winding-below-oil deficit is **not a calibration offset**. It shrinks monotonically with load and **crosses zero inside the measured hull** — from -7.06 K at 0.05-0.10 pu to +2.27 K at 0.30-0.43 pu for Hot-spot HV. A fixed offset cannot do that | **(a)** | 6,247 quasi-steady samples, six load bands. The author independently stated he has no reason to believe an offset error exists; the data agrees with him |
| 127 | The mechanism is sensor **height in the oil column**. Fitted C is **8.93-10.41 K** across all four winding channels against a measured mean top-minus-bottom oil span of **10.37 K**, and channels known to sit lower show a larger C | **(a)** for the numbers; **(c)** for the attribution | Placement per the ARWtr-family publication: sensors at the hot spot and mid-winding of both windings, oil measured into and out of the windings |
| 128 | **The crossing load is identified where the rated gradient is not.** Sweeping the exponent from y = 1.0 to 2.0 moves the fitted rated gradient from 40.1 to 99.1 K — a factor of 2.5 — while K* moves only **0.261 to 0.268 pu, 2.7 %**. The residual barely changes, so the record cannot even choose y | **(a)** | The identifiable quantity is the ratio `C/Delta_hr = K*^y`, not either parameter. Confirmed independently by the free-exponent production fit, which lands at y = 0.63-0.98 and still returns K* = 0.261 |
| 129 | K* orders the four sensors by height from operating data alone: Hot-spot LV 0.220, Hot-spot HV 0.262, Mid-spot LV 0.323, Mid-spot HV 0.376 pu | **(a)** | Correct physical ordering recovered with no placement metadata. A probe-placement diagnostic from load, top oil and one winding channel |
| 130 | **Unexplained and worth pursuing.** If the hot-spot probe sits at the designed hot spot and `Top oil` is winding-outlet oil, C for that channel should be 1-2 K. It fits at **8.93-9.21 K**, most of the 10.37 K column | **(a)** for the discrepancy; **(c)** for the causes | Three candidates, undecided here: the top-oil reference is hotter than the probe's local oil; the probes are lower than "hot spot" implies; or `Delta_hr*K^y` does not hold at 0.05-0.43 pu in ONAN, where the assumed oil profile may not be established |
